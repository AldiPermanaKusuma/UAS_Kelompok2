...
function save_comment($pdo, $user_id, $post_id, $comment_content)
{
    $sql = "INSERT INTO comment_table (user_id, post_id, content, created_at) VALUES (?, ?, ?, NOW())";
    $stmt = $pdo->prepare($sql);
    return $stmt->execute([$user_id, $post_id, $comment_content]);
}
